...
namespace DBow {
	...
	public:
		 ...
		 */
		virtual void Create(const std::vector<std::vector<float> >& training_features) = 0;
		 ...
		 */
		void Transform(const std::vector<float>& features, BowVector &v, bool arrange = true) const;
	...
	protected:
		 ...
		 * @return word id
		 */
		virtual WordId Transform(const std::vector<float>::const_iterator &pfeature) const = 0;
		 ...
		 * @param f (in/out) file
		 */
		void SaveTextHeader(std::fstream &f) const;
		 ...
		 * @return number of words
		 */
		int LoadTextHeader(std::fstream &f);
	...
	protected:
		 ...
		 */
		void GetWordWeightsAndCreateStopList(const std::vector<std::vector<float> >& training_features,
			std::vector<WordValue> &weights);
	...
	protected:
		...
		std::vector<float> m_word_frequency;
