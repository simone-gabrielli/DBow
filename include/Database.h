...
public:
	 ...
	 */
	EntryId AddEntry(const std::vector<float> &features);
	 ...
	 */
	void Query(QueryResults &ret, const std::vector<float> &features, 
...
protected:
	...
	typedef std::list<IFEntry> IFRow;

	// InvertedFile[wordid] = [ <docid,value>, ... ]
	class InvertedFile: public std::vector<IFRow>
	{
	public:
		InvertedFile(){}
		~InvertedFile(){}
...
inline DBow::EntryId DBow::Database::AddEntry(const std::vector<float>& features)
{
	DBow::BowVector v;
...
inline void
DBow::Database::Query(DBow::QueryResults &ret, const std::vector<float> &features, 
